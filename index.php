...
function handleConvert(): array {
    ...
    // ── Sanitise options ──────────────────────────────────────────────────
    $dialect    = in_array($_POST['dialect'] ?? 'mysql', ['sqlite', 'mysql'], true)
                  ? $_POST['dialect'] : 'mysql';
    ...
    $tmpIn  = tempnam($tmpDir, 'sq_in_');
    $tmpOut = tempnam($tmpDir, 'sq_out_');
    ...
    $warnings = substr_count($sql, '-- AVISO:');
...
function renderResult(d) {
  ...
  const chips = [
    ...
    ['Tamanho', d.size_fmt],
    ['Node',    d.node],
    ['Avisos',  (d.warnings ?? 0).toLocaleString('pt')],
  ];
