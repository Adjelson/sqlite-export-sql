...
function validateMysqlOutput(sql) {
  ...
  assertContains(sql, "SET FOREIGN_KEY_CHECKS = @OLD_FOREIGN_KEY_CHECKS;", "restores FOREIGN_KEY_CHECKS");
  ...
  assertNotContains(executableSql, " TEXT ",       "no bare TEXT column type");
  assertNotContains(executableSql, " REAL ",       "no bare REAL column type");
  assertNotContains(executableSql, " BLOB ",       "no bare BLOB column type");
  assertNotContains(executableSql, " BOOLEAN ",    "no bare BOOLEAN column type");
  ...
  // Partial indexes are skipped rather than broadened into a different rule.
  assertContains(sql, "Índice parcial idx_products_active foi ignorado",
    "partial index skipped with explicit warning");
  assert(!/^CREATE.*INDEX.*idx_products_active/m.test(sql),
    "partial index is not emitted as an unsafe global index");
  ...
  // Generated columns remain in DDL but are excluded from INSERT.
  const inventoryInsert = sql.match(/^INSERT INTO `inventory`[^\n]*/m)?.[0] ?? "";
  assertContains(sql, "`subtotal` DOUBLE GENERATED ALWAYS AS",
    "generated STORED column preserved in DDL");
  assertContains(sql, "`label` LONGTEXT GENERATED ALWAYS AS",
    "generated VIRTUAL column preserved in DDL");
  assertNotContains(inventoryInsert, "`subtotal`", "generated STORED col excluded from INSERT");
  assertNotContains(inventoryInsert, "`label`",    "generated VIRTUAL col excluded from INSERT");
  ...
  assert(
    /updated_at`?\s+DATETIME\s+NOT\s+NULL\s+DEFAULT\s+CURRENT_TIMESTAMP/i.test(eventsTable),
    "TEXT datetime column converted to DATETIME with default preserved"
  );
  assert(/DEFAULT\s+\(?'pending'\)?/i.test(eventsTable),
    "text literal DEFAULT preserved safely");
