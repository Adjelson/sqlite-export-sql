...
export function sqlEscape(value, dialect = "sqlite") {
  ...
  if (dialect === "mysql") {
    // Control characters are emitted as UTF-8 hex to avoid broken SQL files.
    if (/[\u0000\u001a\r\n]/.test(s)) {
      return `CONVERT(X'${Buffer.from(s, "utf8").toString("hex")}' USING utf8mb4)`;
    }
    // The generated header disables NO_BACKSLASH_ESCAPES for this session.
